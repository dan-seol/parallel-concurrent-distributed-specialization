...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int endRow = (myRank + 1) * rowChunk > nRows ? nRows : (myRank + 1) * rowChunk;

        //broadcast all the matcies a & b to all ranks
        // more efficient approach - 0 only send the relevant rows and columns
        ...
        if (myRank == 0) {
            ...
            for (int i = 1; i < size; i++) {
                ...
                final int rankEndRow = (i + 1) * rowChunk > nRows ? nRows : (i + 1) * rowChunk;
